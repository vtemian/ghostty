...
fn initPager(alloc: Allocator) ?std.process.Child {
    ...
    var env = compat_env.getEnvMap(alloc) catch return null;
    defer env.deinit();
    ...
    const env_block: std.process.Environ.Block = switch (builtin.os.tag) {
        .linux, .freebsd, .macos => env.createPosixBlock(alloc, .{}) catch return null,
        .windows => env.createWindowsBlock(alloc, .{}) catch return null,
        else => return null, // unsupported OS
    };
